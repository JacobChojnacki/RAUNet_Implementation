...
def simple_unet_model(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS):
    ...
    b1 = conv_block(x=p4, num_filters=1024, kernel_size=3, dropout=0, batch_normalization=True)  # Bridge
...
def res_conv_block(x, num_filters, kernel_size, dropout, batch_normalization=True):
    conv = Conv2D(num_filters, (kernel_size, kernel_size),
                  padding='same')(x)
    if batch_normalization:
        conv = BatchNormalization()(conv)
...
def Attention_unet_model(shape=(128, 128, 3),
    ...
    b1 = conv_block(x=p4, num_filters=num_filters * filter_multiplier[3], kernel_size=kernel_size, dropout=dropout, batch_normalization=batch_norm)  # Bridge

    gatting_4 = gating_signal(b1, num_filters * filter_multiplier[2], batch_norm)
    att_4 = attention_block(s4, gatting_4, num_filters * filter_multiplier[2], name='attention_1')
    ...
    att_3 = attention_block(s3, gating_3, num_filters * filter_multiplier[1], name='attention_2')
    ...
    att_2 = attention_block(s2, gating_2, num_filters * filter_multiplier[0], name='attention_3')
...
# ------------------------------------------------ RESIDUAL NETWORK -------------------------------------------------#
def encoder_residual_block(input, num_filters, kernel_size=3,batch_norm=True, dropout=0):
...
def Attention_residual_unet_model(shape=(128, 128, 3),
    ...
    s1, p1 = encoder_residual_block(inputs, num_filters, kernel_size=kernel_size, batch_norm=batch_norm, dropout=dropout)
    ...
    d1 = decoder_residual_block(b1, att_4, filter_multiplier[2] * num_filters, batch_norm=batch_norm, dropout=0.1)
    ...
    d2 = decoder_residual_block(d1, att_3, filter_multiplier[1] * num_filters, batch_norm=batch_norm, dropout=0.1)
    ...
    d3 = decoder_residual_block(d2, att_2, filter_multiplier[0] * num_filters, batch_norm=batch_norm, dropout=0.1)

    gating_1 = gating_signal(d3, num_filters, batch_norm)
    att_1 = attention_block(s1, gating_1, num_filters, name="attention_4")
    d4 = decoder_residual_block(d3, att_1, num_filters, batch_norm=batch_norm, dropout=0.1)
